...
const totalCars = users.reduce((sum, user) => sum + (user.cars ? user.cars.length : 0), 0);
...
//  3. Создать функцию, которая бы принимала массив пользователей и отфильтровывала пользователей на наличие образования
function filterEducatedUsers(users) {
...
//4. Создать функцию, которая бы принимала массив пользователей и отфильтровывала пользователей на наличие животных:
function filterUsersWithAnimals(users) {
    const filteredUsersWithAnimals = [];
...
const carsString = getCars(users);
